<?php
// -----------------------------------------------------------------------------
// EduuAspire Corporate Onboarding & Skilling | Goa Workforce Alignment
// Debug + Auto Populate + Dynamic Filter Version (PHP 5.4 compatible)
// -----------------------------------------------------------------------------
...
$sector     = norm($_POST['sector']);
$training   = norm($_POST['training']);
...
// -----------------------------------------------------------------------------
$check = $coni->query("SELECT COUNT(*) AS c FROM lessons WHERE LOWER(academic_level)='corporate'");
$count = $check->fetch_assoc();

if (intval($count['c']) == 0) {
  $sectors = array(
    "Hospitality & Tourism" => array("Front Office Onboarding","Housekeeping Essentials","Food & Beverage Service","Hospitality Etiquette","Safety & Hygiene Induction"),
    "Industrial & Manufacturing" => array("Industrial Safety Induction","Machine Operations Basics","Electrical & Maintenance","Supervisory Skills Development"),
    "Security & Allied Services" => array("Basic Security Training","Emergency Response & First Aid","Fire Safety & Evacuation Drill","Crowd Management Skills"),
    "Retail & Customer Care" => array("POS Handling","Customer Communication Skills","Sales Etiquette","Complaint Management"),
    "Facility & Logistics" => array("Material Handling Safety","Warehouse Operations","Driver Induction Program","Fleet & Transport Management")
  );
  ...
  foreach ($sectors as $sectorName => $courses) {
    foreach ($courses as $c) {
      ...
      $rating = round((rand(43,49)/10),1);
      $reviews = rand(15,80);

      $info = "Corporate onboarding training for {$sectorName}: {$c}, delivered in {$modeType} mode with {$assessmentType} evaluation.";
      $stmt = $coni->prepare("INSERT INTO lessons 
        ...
        VALUES (?, ?, ?, ?, ?, 0.00, 'Paid', 'Corporate', ?, ?, 'Online', ?, 1, 1, 1, UNIX_TIMESTAMP())");
...
// Dynamic Fetch Query
// -----------------------------------------------------------------------------
$q = "SELECT id, name, info, board AS sector, course_mode, assessment_type, learners, rating, reviews,
CASE
...
FROM lessons
WHERE active=1 AND publish=1 AND LOWER(academic_level)='corporate'";

$p = array(); $t = '';

if ($sector && $sector != 'all') { $q .= " AND LOWER(board)=?"; $p[] = $sector; $t .= 's'; }
if ($training && $training != 'all') { $q .= " AND LOWER(name) LIKE ?"; $p[] = "%".$training."%"; $t .= 's'; }
...
if (isset($_GET['debug'])) {
  echo "<pre>Query: $q\n\nParams: "; print_r($p); echo "\n\nCourses Found: ".count($courses)."</pre>";
  die;
}
